...
bool FC::init(void *data, udword length, int startStep, int endStep) {
    ...
    inputLen = length;
#endif
    fcBufS = reinterpret_cast<int8_t *>(data);
    fcBuf = reinterpret_cast<uint8_t *>(data);
...
#ifndef FC14_USE_OG
    fc14header *header = reinterpret_cast<fc14header *>(data);
#endif
    ...
    _admin.offsets.patterns = readEndian(fcBuf[8],fcBuf[9],
                                           fcBuf[10],fcBuf[11]);
#else
    _admin.offsets.patterns = header->patternDataOffset;
#endif
...
#else
    _admin.usedPatterns = header->patternDataSize / PATTERN_LENGTH;
#endif
#if defined(DEBUG)
    ...
    _admin.offsets.sndModSeqs = readEndian(fcBuf[16],fcBuf[17],
                                            fcBuf[18],fcBuf[19]);
#else
    _admin.offsets.sndModSeqs = header->frequencySequenceOffset;
#endif
...
#else
    _admin.usedSndModSeqs = header->frequencySequenceSize / 64;
#endif
#if defined(DEBUG)
    ...
    _admin.offsets.volModSeqs = readEndian(fcBuf[24],fcBuf[25],
                                            fcBuf[26],fcBuf[27]);
#else
    _admin.offsets.volModSeqs = header->volumeSequenceOffset;
#endif
...
#else
    _admin.usedVolModSeqs = header->volumeSequenceSize / 64;
#endif
#if defined(DEBUG)
    ...
    udword sampleOffset = readEndian(fcBuf[32],fcBuf[33],
                                     fcBuf[34],fcBuf[35]);
#else
    udword sampleOffset = header->sampleDataOffset;
#endif
...
#else
        struct alignas(2) Fc14SampleHeader {
            uint16_t sampleLength;
            uint16_t repetitionOffset;
            uint16_t repetitionLength;
        } *sh = reinterpret_cast<Fc14SampleHeader *>(reinterpret_cast<uint8_t *>(data) + sampleHeader);
        uword sampleLength = sh->sampleLength;
        _sounds[sam].start = sampleOffset + fcBuf;
        _sounds[sam].len = sh->sampleLength;
        _sounds[sam].repOffs = sh->repetitionOffset;
        _sounds[sam].repLen = sh->repetitionLength;
#endif
...
#ifdef FC14_USE_OG // Don't write, our sample data is in ROM
        if (isSMOD) {
    ...
    if (isSMOD) {
        ...
        udword waveOffset = readEndian(fcBuf[36],fcBuf[37],
                                       fcBuf[38],fcBuf[39]);
#else
        udword waveOffset = header->waveformDataOffset;
#endif
...
#else
            _sounds[sam].start = waveOffset + fcBuf;
#endif
...
#else
    auto *header = reinterpret_cast<const fc14header *>(fcBuf);
    udword trackTabLen = header->patternIndexTableSize;
#endif
...
#else
    CHXdata.ch->paula.start = fcBuf + _admin.offsets.silence + 1;
#endif
...
#else
            udword sndOffs = _sounds[i].start - fcBuf;
#endif
            // "SSMP"? sample-pack?
...
#else
                CHXdata.pSampleStart = smpStart + fcBuf;
#endif
...
#ifdef FC14_USE_OG // Our data is in ROM, do not write
